...
class OnlisansScrapper(Scrapper):

    def __init__(self, url, dtId, delay, perPage):
        super(OnlisansScrapper, self).__init__(url, dtId, delay, perPage)
    ...
    def format(self, extra_fields = False):
        ...
        for index, cols in enumerate(self.data):
            ...
            yerlesen = BeautifulSoup(cols[9], 'html.parser').find_all('font')
            taban_puan_sirasi = BeautifulSoup(cols[10], 'html.parser').find_all('font')
            taban_puan = BeautifulSoup(cols[11], 'html.parser').find_all('font')
            formattedData = {
